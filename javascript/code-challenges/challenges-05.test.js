...
const reversedString = (str) => {
  let newArray = [...str];
  let revArray = newArray.reduce((acc, val) => {
    acc.unshift(val);//reverse str array
    return acc;
  }, []);

  let arrChar = revArray.reduce((acc, val) => {
    let sum = acc + val;
    return sum;
  }, '');
  return arrChar;
